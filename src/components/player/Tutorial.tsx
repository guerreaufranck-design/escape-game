...
// 5-slide essential tutorial — anything more was too long and players
// skipped it (or read passively). The first slide grabs attention so the
// next four actually land. Order is intentional: WHY read this → AR is
// THE rule → WHERE to tap → HOW to find the clue → SAFETY NET if stuck.
const SLIDES: Slide[] = [
  { titleKey: "tutorial.s1.title", textKey: "tutorial.s1.text", color: "text-orange-400",  icon: <AlertTriangle className="h-10 w-10" /> },
  { titleKey: "tutorial.s2.title", textKey: "tutorial.s2.text", color: "text-fuchsia-400", icon: <Sparkles      className="h-10 w-10" /> },
  { titleKey: "tutorial.s5.title", textKey: "tutorial.s5.text", color: "text-fuchsia-400", icon: <Camera        className="h-10 w-10" /> },
