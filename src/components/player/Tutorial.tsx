...
const getSlides = (estimatedDuration?: string): Slide[] => [
  ...
  // 9. Hints — now help with the SCAN, not the answer.
  {
    ...
    text: {
      fr: "Vous tournez en rond, l'indice ne se montre pas ? Demandez un indice (icone ampoule). Trois indices, de plus en plus precis sur l'EMPLACEMENT : le 1er pointe une zone (\"vers l'entree principale\"), le 2e une surface precise (\"sur la porte en bois\"), le 3e decrit la FORME de la reponse (\"un mot latin de 6 lettres\"). Chaque indice coute du temps. Vraiment bloque ? Vous pouvez aussi passer l'etape (penalite plus lourde, mais la reponse vous est revelee).",
      en: "Going in circles, the clue won't show? Ask for a hint (bulb icon). Three hints, escalating precision about WHERE the answer is hidden: 1st points at a zone (\"near the main entrance\"), 2nd at a specific surface (\"on the wooden door\"), 3rd describes the SHAPE of the answer (\"a 6-letter Latin word\"). Each hint costs time. Really stuck? You can also skip the step (heavier penalty, but the answer is revealed).",
      de: "Drehen sich im Kreis, der Hinweis erscheint nicht? Fordern Sie einen Hinweis an (Gluhbirnen-Symbol). Drei Hinweise mit steigender Prazision zum ORT: 1. zeigt eine Zone (\"in der Nahe des Haupteingangs\"), 2. eine konkrete Flache (\"auf der Holztur\"), 3. beschreibt die FORM der Antwort (\"ein 6-Buchstaben-Latein-Wort\"). Jeder Hinweis kostet Zeit. Wirklich festgefahren? Sie konnen auch die Etappe uberspringen (schwerere Strafe, aber die Antwort wird enthullt).",
      es: "Das vueltas, la pista no aparece? Pide una pista (icono bombilla). Tres pistas, precision creciente sobre el LUGAR: la 1a apunta a una zona (\"cerca de la entrada principal\"), la 2a a una superficie concreta (\"en la puerta de madera\"), la 3a describe la FORMA de la respuesta (\"una palabra latina de 6 letras\"). Cada pista cuesta tiempo. Muy atascado? Tambien puedes saltar la etapa (penalizacion mas fuerte, pero la respuesta se revela).",
      it: "Girate a vuoto, l'indizio non appare? Chiedete un indizio (icona lampadina). Tre indizi, precisione crescente sul LUOGO: il 1o indica una zona (\"vicino all'ingresso principale\"), il 2o una superficie precisa (\"sulla porta di legno\"), il 3o descrive la FORMA della risposta (\"una parola latina di 6 lettere\"). Ogni indizio costa tempo. Davvero bloccati? Potete anche saltare la tappa (penalita piu pesante, ma la risposta viene rivelata).",
    },
  },
