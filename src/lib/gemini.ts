...
/**
 * Translate `text` to `targetLang`. Tries Gemini first (cheaper, faster),
 * falls back to Claude on any Gemini error (quota, key issues, project
 * denied, transient 5xx). The fallback makes the translation pipeline
 * resilient to Gemini outages — the player never sees an English-only
 * page just because Google rate-limited us.
 *
 * `sourceLang` lets the caller hint at the actual source language (e.g.
 * legacy games stored in plain French). Defaults to "en" since virtually
 * all modern game content is generated by Claude in English.
 */
export async function translateText(
  ...
  const prompt = `Translate the following text from ${sourceName} to ${langName}. Return ONLY the translated text, nothing else. Preserve any special markers like [FIELD:xxx] or [KEY:xxx] exactly as they are.\n\n${text}`;

  // Primary: Gemini Flash via REST (cheapest path, ~$0.02/1k chars).
  try {
