/**
 * Wipe the Shibuya "Lucie Blackman" game and every related row across
 * all tables — steps, sessions, completions, hints, photos, feedback,
 * activation codes, audio_cache, translations_cache.
 *
 * Reason: 5 of 8 steps shipped with lat=0 lon=0 (Claude failed to
 * resolve fictional venues, pipeline silently inserted defaults). The
 * radar pointed at the middle of the Atlantic. The new pipeline guard
 * (game-pipeline.ts) refuses such games going forward; this wipes the
 * one corrupt entry already in the DB so it can be regenerated clean.
 */
...
const GAME_ID = "4b1726a4-edac-479f-8104-42cba4a7ddab";
...
async function main() {
  console.log(`Deleting game ${GAME_ID} (Shibuya — broken coords)...`);
  ...
  // Translation cache — game-level + step-level + synthetic hint /
  // attraction keys.
  await report(
  ...
  console.log("\n✅ Done. Shibuya is wiped — safe to regenerate from scratch.");
}
